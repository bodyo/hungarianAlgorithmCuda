...
const int n = 4096;						// size of the cost/pay matrix
...
const int log2_n = 12;					// log2(n) needs to be entered manually
...
{
	// Constant checks:
	check(n == (1 << log2_n), (char*)"Incorrect log2_n!");
	check(n_threads*n_blocks == n, (char*)"n_threads*n_blocks != n\n");
	// step 1
	check(n_blocks_reduction <= n, (char*)"Step 1: Should have several lines per block!");
	check(n % n_blocks_reduction == 0, (char*)"Step 1: Number of lines per block should be integer!");
	check((n_blocks_reduction*n_threads_reduction) % n == 0, (char*)"Step 1: The grid size must be a multiple of the line size!");
	check(n_threads_reduction*n_blocks_reduction <= n*n, (char*)"Step 1: The grid size is bigger than the matrix size!");
	// step 6
	check(n_threads_full*n_blocks_full <= n*n, (char*)"Step 6: The grid size is bigger than the matrix size!");
